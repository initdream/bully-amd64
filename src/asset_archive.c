...
static const char *g_idx_files[] = {
  "data_0.zip.idx", "data_1.zip.idx", "data_2.zip.idx", "data_3.zip.idx", "data_4.zip.idx",
};
...
static void normalize_path(char *dst, size_t dst_size, const char *src) {
  ...
  while ((*src == '.' && src[1] == '/') || *src == '/') src += (*src == '/') ? 1 : 2;
...
static int add_zip_index_alias(const char *zip_name, const char *path, uint32_t data_offset, uint32_t size) {
  ...
  ZipIndexEntry *entry = &g_zip_entries[g_zip_entry_count++];
  size_t len = strlen(path);
  entry->path = malloc(len + 1);
  if (!entry->path) { g_zip_entry_count--; return -1; }
...
static int load_zip_index_file(const char *idx_name) {
  ...
  fseek(fp, 0, SEEK_SET);
  if (file_size < 4) { fclose(fp); return -1; }

  unsigned char *data = malloc((size_t)file_size);
  if (!data) { fclose(fp); return -1; }

  if (fread(data, 1, (size_t)file_size, fp) != (size_t)file_size) { free(data); fclose(fp); return -1; }
  fclose(fp);
  ...
  uint32_t entry_count = read_u32le(data);
  size_t pos = 4;
  for (uint32_t i = 0; i < entry_count; i++) {
    if (pos + 10 > (size_t)file_size) break;
    uint32_t data_offset = read_u32le(&data[pos + 0]);
    uint32_t size = read_u32le(&data[pos + 4]);
    uint16_t name_len = read_u16le(&data[pos + 8]);
    pos += 10;

    if (pos + name_len > (size_t)file_size || name_len >= 512) break;
    char path_buf[512], alias_buf[512];
    memcpy(path_buf, &data[pos], name_len);
    path_buf[name_len] = '\0';
    pos += name_len;

    normalize_path(alias_buf, sizeof(alias_buf), path_buf);
    if (add_zip_index_alias(zip_name, alias_buf, data_offset, size) < 0) break;

    if (strncmp(alias_buf, "bullyorig/", 10) == 0) {
      if (add_zip_index_alias(zip_name, alias_buf + 10, data_offset, size) < 0) break;
    } else if (strncmp(alias_buf, "bully/", 6) == 0) {
      if (add_zip_index_alias(zip_name, alias_buf + 6, data_offset, size) < 0) break;
    }
  }
  free(data);
  return 0;
...
static const ZipIndexEntry *find_zip_entry(const char *normalized_path) {
  ...
  return (const ZipIndexEntry *)bsearch(&key, g_zip_entries, g_zip_entry_count, sizeof(ZipIndexEntry), cmp_zip_entry);
}

static AssetHandle *asset_handle_from_file(FILE *fp) {
  if (!fp) return NULL;
  AssetHandle *handle = calloc(1, sizeof(*handle));
  if (!handle) { fclose(fp); return NULL; }
...
static AssetHandle *asset_handle_from_slice(const char *zip_name, uint32_t start, uint32_t size) {
  ...
  if (fseek(fp, (long)start, SEEK_SET) == 0) { fread(magic, 1, 32, fp); }

  if (magic[0] == 'P' && magic[1] == 'K' && magic[2] == 3 && magic[3] == 4) {
    uint16_t fn_len = (uint16_t)(magic[26] | (magic[27] << 8));
    uint16_t extra_len = (uint16_t)(magic[28] | (magic[29] << 8));
    uint32_t header_size = 30u + fn_len + extra_len;
    start += header_size;
  }
...
static int load_img_pack(ImgPack *pack) {
  const ZipIndexEntry *dir_entry = find_zip_entry(pack->dir_path);
  const ZipIndexEntry *img_entry = find_zip_entry(pack->img_path);
  if (!dir_entry || !img_entry) return -1;

  AssetHandle *dir_handle = asset_handle_from_slice(dir_entry->zip_name, dir_entry->data_offset, dir_entry->size);
  if (!dir_handle) return -1;

  unsigned char *dir_data = malloc(dir_entry->size);
  if (!dir_data) { asset_close(dir_handle); return -1; }

  if (asset_read(dir_data, 1, dir_entry->size, dir_handle) != dir_entry->size) {
    free(dir_data); asset_close(dir_handle); return -1;
  }
  asset_close(dir_handle);

  size_t count = dir_entry->size / 32;
  pack->entries = calloc(count, sizeof(*pack->entries));
  if (!pack->entries) { free(dir_data); return -1; }
...
int asset_archive_init(void) {
  if (g_asset_archive_attempted) return g_asset_archive_ready;
  g_asset_archive_attempted = 1;

  for (size_t i = 0; i < sizeof(g_idx_files) / sizeof(g_idx_files[0]); i++)
    load_zip_index_file(g_idx_files[i]);

  for (size_t i = 0; i < sizeof(g_img_packs) / sizeof(g_img_packs[0]); i++)
...
static AssetHandle *open_loose_asset(const char *path, const char *normalized_path) {
  ...
  FILE *fp = fopen(fullpath, "rb");
  if (fp) return asset_handle_from_file(fp);
...
static const ImgDirEntry *find_img_entry(const ImgPack *pack, const char *name) {
  for (size_t i = 0; i < pack->entry_count; i++) {
    if (strcmp(pack->entries[i].name, name) == 0) return &pack->entries[i];
  }
  return NULL;
}
...
static AssetHandle *open_img_pack_asset(const char *path, const char *normalized_path) {
  ...
  for (size_t i = 0; i < sizeof(g_img_packs) / sizeof(g_img_packs[0]); i++) {
    ...
    if (pack->entry_count == 0) continue;
    if (strncmp(normalized_path, pack->logical_prefix, strlen(pack->logical_prefix)) != 0) continue;
    const ImgDirEntry *entry = find_img_entry(pack, basename);
    if (!entry) return NULL;
  ...
  if (!fallback_pack || !fallback_entry) return NULL;
...
void *asset_open(const char *path) {
  ...
  AssetHandle *handle = open_loose_asset(path, normalized_path);
  if (!handle) handle = open_zip_asset(normalized_path);
  if (!handle) handle = open_img_pack_asset(path, normalized_path);
...
void asset_close(void *opaque) {
  ...
  if (!handle || handle->magic != HANDLE_MAGIC) return;
...
size_t asset_read(void *buf, size_t size, size_t nmemb, void *opaque) {
  AssetHandle *handle = (AssetHandle *)opaque;
  if (!handle || handle->magic != HANDLE_MAGIC || !buf || size == 0 || nmemb == 0) return 0;

  pthread_mutex_lock(&g_asset_lock);
  size_t result = 0;
  size_t req_total = size * nmemb;

  if (handle->kind == ASSET_HANDLE_FILE) {
    result = fread(buf, size, nmemb, handle->fp);
  } else if (handle->pos < handle->size) {
    long remaining = handle->size - handle->pos;
    size_t to_read = (size_t)((remaining < (long)req_total) ? remaining : (long)req_total);
    if (fseek(handle->fp, handle->start + handle->pos, SEEK_SET) == 0) {
      size_t bytes_read = fread(buf, 1, to_read, handle->fp);
...
int asset_seek(void *opaque, long offset, int whence) {
  AssetHandle *handle = (AssetHandle *)opaque;
  if (!handle || handle->magic != HANDLE_MAGIC) return -1;

  pthread_mutex_lock(&g_asset_lock);
  int res = -1;
  if (handle->kind == ASSET_HANDLE_FILE) {
    res = fseek(handle->fp, offset, whence);
  } else {
    long new_pos = -1;
    switch (whence) {
      case SEEK_SET: new_pos = offset; break;
      case SEEK_CUR: new_pos = handle->pos + offset; break;
      case SEEK_END: new_pos = handle->size + offset; break;
    }
    if (new_pos >= 0) { handle->pos = new_pos; res = 0; }
  }
  pthread_mutex_unlock(&g_asset_lock);
  return res;
}
...
long asset_tell(void *opaque) {
  ...
  if (!handle || handle->magic != HANDLE_MAGIC) return -1;
  pthread_mutex_lock(&g_asset_lock);
  long pos = (handle->kind == ASSET_HANDLE_FILE) ? ftell(handle->fp) : handle->pos;
  pthread_mutex_unlock(&g_asset_lock);
  return pos;
}
...
long asset_size(void *opaque) {
  AssetHandle *handle = (AssetHandle *)opaque;
  if (!handle || handle->magic != HANDLE_MAGIC) return 0;
  if (handle->kind == ASSET_HANDLE_SLICE) return handle->size;
  long cur = ftell(handle->fp);
...
int asset_eof(void *opaque) {
  AssetHandle *handle = (AssetHandle *)opaque;
  if (!handle || handle->magic != HANDLE_MAGIC) return 1;
  if (handle->kind == ASSET_HANDLE_FILE) return feof(handle->fp);
  return (handle->pos >= handle->size) ? 1 : 0;
}

int asset_getc(void *opaque) {
  unsigned char c;
  if (asset_read(&c, 1, 1, opaque) != 1) return -1;
  return c;
}
